#!/usr/bin/env python3
"""Wait until the persistent YOLO-World service has loaded its checkpoint."""
...
def probe(endpoint: str, timeout_s: float, worker_pid: int | None = None) -> dict:
    ...
    while time.monotonic() < deadline:
        if worker_pid is not None:
            try:
                ...
                raise RuntimeError("yolo_world_worker_exited") from exc
        connection = None
        try:
            ...
            if not connection.poll(max(0.1, deadline - time.monotonic())):
                raise TimeoutError("yolo_healthcheck_timeout")
            ...
            if response.get("request_id") != request_id or response.get("ok") is not True:
                raise RuntimeError(str(response.get("error_code") or "yolo_healthcheck_failed"))
    ...
    if last_error is not None:
        raise RuntimeError(f"yolo_healthcheck_unreachable:{type(last_error).__name__}") from last_error
    raise TimeoutError("yolo_healthcheck_timeout")


def main() -> int:
    parser = argparse.ArgumentParser()
    parser.add_argument("--endpoint", default="@mast3r_yolo")
